...
def plot_result_cf(testdf, y_pred, missdf, traindf, y_train):
    fig = plt.figure()
    ax = fig.gca()
    yp = testdf['y_plus']
    y_pred = [int(i) for i in y_pred]
    cols = get_cmap(19)
    scatter = ax.scatter(yp, y_pred, color=cols(y_pred), s=1)
    yp = traindf['y_plus']
    y_train = [int(i) for i in y_train]
    scatter = ax.scatter(yp, y_train, color=cols(y_train), s=1)
    yp = missdf['y_plus']
    ymiss = missdf['cluster.ID']
    scatter = ax.scatter(yp, ymiss, color='black', s=1)
    plt.xscale('log')
    plt.xlim([1, 5000])
    plt.ylim([0, 6])
    plt.xlabel('$y^{+}$')
...
def plot_result(testdf, y_pred, missdf, traindf, y_train):
    fig = plt.figure()
    ax = fig.gca()
    if ('x' in testdf):
        x = testdf['x']
        ...
        y_pred = [int(i) for i in y_pred]
        cols = get_cmap(19)
        scatter = ax.scatter(x, y, color=cols(y_pred), s=0.05)
        x = traindf['x']
        y = traindf['y']
        y_train = [int(i) for i in y_train]
        scatter = ax.scatter(x, y, color=cols(y_train), s=0.05)
        x = missdf['x']
        y = missdf['y']
        scatter = ax.scatter(x, y, color='black', s=1)
        plt.xlabel('x')
        plt.ylabel('y')
        plt.show()
    else:
        plot_result_cf(testdf, y_pred, missdf, traindf, y_train)
    return
...
def lda_rf_body(cdf, train_index, test_index, features, classes):
    ...
    missed, y_pred2, missdf = compute_accuracy(ccdf, exptdf, missdf, delta, classes)
    y_pred = y_pred2[test_index]
    plot_result(exptdf, y_pred2.to_list(), missdf, ccdf_train, y_train)
    macc = accuracy_score(y_test, y_pred)
